...
for(const preset of ['front','left','right','back']){engine.preset(preset);frame();finiteCamera();report.totalCheckedFrames++;check(!box.containsPoint(camera.position),'Preset places camera within subject AABB',{id,preset,position:camera.position.toArray()});
...
// Presets cover every side; several real key events verify constrained rotation as well.
...
// Real pointer selection on a close-up must trigger an object action, not content.
canvas.clientWidth=1280;canvas.clientHeight=800;window.dispatchEvent(new Event('resize'));engine.reset();engine.setPaused(false);frame();
const smallBird=engine.getActions().find(a=>a.kind==='bird');
assert.ok(engine.activate(smallBird.id));frame(24);
assert.equal(reported.action,smallBird.id);
let owned;room.traverse(o=>{if(o.userData.roomActionId===smallBird.id)owned=o;});
const headPart=owned.getObjectByName('Songbird turning head'),hitPoint=headPart.getWorldPosition(new T.Vector3()).project(camera);
const birdXY={clientX:(hitPoint.x+1)*640,clientY:(1-hitPoint.y)*400};
let actionEvents=0;const counted=()=>actionEvents++;canvas.addEventListener('viewchange',counted);
const priorSelects=selected.length;send('pointerdown',birdXY);send('pointerup',birdXY);
check(actionEvents===1&&selected.length===priorSelects,'A bird tap must trigger only its own action',{actionEvents});
const eventsAfterTap=actionEvents;send('pointerdown',birdXY);send('pointermove',{...birdXY,clientX:birdXY.clientX+30});send('pointerup',{...birdXY,clientX:birdXY.clientX+30});
check(actionEvents===eventsAfterTap,'Dragging a bird must not replay its action',{actionEvents});
send('pointerdown',birdXY);send('pointercancel',birdXY);send('pointerup',birdXY);
check(actionEvents===eventsAfterTap,'Cancelled tap triggers object action',{actionEvents});
engine.setPaused(true);frame();const heldPose=headPart.quaternion.clone();engine.activate(smallBird.id);frame(12);
check(heldPose.angleTo(headPart.quaternion)<1e-7,'Reduced/paused motion still animates bird',{});
send('keydown',{key:'Escape'});frame();check(!reported.action&&reported.subject==='room','Escape fails to return from object close-up',{});
engine.inspect(ids[0]);frame();engine.activate(smallBird.id);frame();
check(visible(room)&&reported.action===smallBird.id,'Object selector cannot return from isolated model inspection',{});

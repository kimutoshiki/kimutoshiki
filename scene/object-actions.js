...
export function createObjectActions(T, root){
  const entries=new Map(),running=new Map(),applied=[];
  ...
  return {
    ...
    trigger(id){const entry=entries.get(id);if(!entry)return null;running.clear();running.set(id,{entry,elapsed:0});return entry;},
    clear(){restore();running.clear();},
    update(delta,enabled=true){
      if(!enabled){running.clear();return false;}

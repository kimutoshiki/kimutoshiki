...
export async function mountStudy({canvas,pins,onSelect,onReady,onError}) {
  ...
  let width=innerWidth,height=innerHeight,portrait=false,active=null,hover=null,night=false,nightMix=0,paused=software||matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  function setInspection(id){
    ...
    scene.background.set(inspecting?'#302d27':'#a9a077');
    if(inspecting){

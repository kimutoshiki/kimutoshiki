...
assert.ok(room.group.getObjectByName('Four walls · four atlases'),'All four supplied atlas wallpapers are present');
assert.deepEqual(room.photoWall.atlas.panels.map(panel=>panel.id),['karatsu','world','japan','europe']);
...
const seatedPosition=new T.Vector3(0,3.48,8.24);
...
assert.ok(clockBounds.getSize(new T.Vector3()).x>1.1&&clockBounds.getSize(new T.Vector3()).x<1.2&&clockBounds.min.y>3.9,'Clock stays compact, elevated and clear of the navigation');

// Only two photo requests run at once; four front photos plus Karatsu gate readiness.
const journeyRequests=()=>requests.filter(request=>request.url.includes('/images/journeys/'));
assert.equal(journeyRequests().length,2);assert.equal(photos.filter(frame=>frame.queued).length,4);
let completed=0;
...
assert.equal(room.photoWall.frontProgress,1);assert.equal(room.photoWall.photosLoaded,4);assert.equal(journeyRequests().length,4);
assert.equal(room.photoWall.atlas.panels.filter(panel=>panel.loaded).length,1,'Other wall maps remain deferred');
...
assert.equal(additional,6,'Turning toward a new wall starts two additional loads');
room.photoWall.update(camera);assert.equal(journeyRequests().length,additional,'An unchanged view does not duplicate loads');
completeRequests();assert.ok(room.photoWall.photosLoaded>4&&room.photoWall.photosLoaded<30,'Only the revealed wall is decoded');
...
const decorRequests=requests.filter(r=>r.url.includes('/images/decor/')&&!r.url.includes('window-garden'));assert.equal(decorRequests.length,4,'Only textile surfaces remain external decor images');

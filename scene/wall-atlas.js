...
export function createWallAtlas(T, { room, palette, onTextureLoad = () => {} }) {
  ...
  let disposed = false, activeLoads = 0, frontSettled = false, resolveReady;
  ...
  function enqueue(panel) { if (panel.queued) return; panel.queued = true; queue.push(panel); }
  for (const [id, spec] of Object.entries(ATLAS_WALLS)) {
    ...
    const record = { id, wall: spec.wall, panel, image, src: '/images/atlas/' + spec.file + '.webp', priority: id === 'karatsu', queued: false, loaded: false };
    panels.push(record); if (record.priority) enqueue(record);
  }
  pump();
  const point = new T.Vector3(), normal = new T.Vector3(), towardCamera = new T.Vector3();
  return {
    group, panels, ready,
    ...
    update(camera, frustum) {
      ...
      for (const panel of panels) {
        ...
        panel.panel.getWorldPosition(point); normal.set(0, 0, 1).transformDirection(panel.panel.matrixWorld);
        towardCamera.copy(camera.position).sub(point);
        if (normal.dot(towardCamera) > 0 && frustum.intersectsObject(panel.image)) enqueue(panel);
      }
      pump();
    },

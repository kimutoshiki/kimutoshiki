...
export class CampusCanvasRenderer {
 ...
 const indices=index?index.count:attr.count;const base=material.color.clone().convertLinearToSRGB(),vertexColors=material.vertexColors?g.getAttribute('color'):null,faceColor=new T.Color();
 for(let i=0;i<indices;i+=3){v0.fromBufferAttribute(attr,index?index.getX(i):i).applyMatrix4(world);v1.fromBufferAttribute(attr,index?index.getX(i+1):i+1).applyMatrix4(world);v2.fromBufferAttribute(attr,index?index.getX(i+2):i+2).applyMatrix4(world);edge1.subVectors(v1,v0);edge2.subVectors(v2,v0);normal.crossVectors(edge1,edge2).normalize();const brightness=.72+Math.max(0,normal.dot(this.light))*.29;let colorBase=base;if(vertexColors){const ids=[0,1,2].map(j=>index?index.getX(i+j):i+j);faceColor.setRGB(ids.reduce((s,k)=>s+vertexColors.getX(k),0)/3,ids.reduce((s,k)=>s+vertexColors.getY(k),0)/3,ids.reduce((s,k)=>s+vertexColors.getZ(k),0)/3).multiply(material.color).convertLinearToSRGB();colorBase=faceColor;}const r=Math.min(255,Math.round(colorBase.r*255*brightness)),gg=Math.min(255,Math.round(colorBase.g*255*brightness)),b=Math.min(255,Math.round(colorBase.b*255*brightness));triangles.push({a:v0.clone(),b:v1.clone(),c:v2.clone(),normal:normal.clone(),rgba:(255<<24)|(b<<16)|(gg<<8)|r});}
 }
